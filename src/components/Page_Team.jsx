<!DOCTYPE html>
<html lang="zxx">
    <head>
        <!-- top scripts/meta data -->
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <title>CiJo</title>
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <!-- Favicon -->
        <!-- <link rel="shortcut icon" href="images/favicon.png"> -->

        <!-- CSS files-->
        <link rel="stylesheet" href="assets/css/main.css?v=1" />
    </head>
    <body>   
        <!-- header -->
        <header>
            <div class="container cijo-main-container">
                <!-- row -->
                <div class="row">
                    <div class="col-12">
                        <nav class="navbar navbar-expand-lg navbar-light">
                            <a class="navbar-brand" href="Home"><img src="assets/images/cijo-logo.svg" class="cijo-main-logo" alt="cijo-logo"/></a>
                            <a href="#" class="cijo-button-burger" ><span>Menu</span></a>
                            <div class="collapse navbar-collapse" id="navbarMain">
                                <ul class="navbar-nav">
                                    <li class="nav-item active">
                                        <a class="nav-link" href="01.Home.html">HOME <span class="sr-only">(current)</span></a>
                                    </li>
                                    <li class="nav-item dropdown">
                                        <!-- <a class="nav-link dropdown-toggle" href="#" id="dropdown01" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">PAGES</a> -->
                                        <a class="nav-link dropdown-toggle" href="#" id="dropdown01" >PAGES</a>
                                        <div class="dropdown-menu" aria-labelledby="dropdown01">
                                            <a class="dropdown-item" href="02.About.html">ABOUT</a>
                                            <a class="dropdown-item" href="09.Team.html">TEAM</a>
                                            <a class="dropdown-item" href="10.Services.html">SERVICES</a>
                                            <a class="dropdown-item" href="08.FAQ.html">FAQ</a>
                                        </div>
                                    </li>
                                    <li class="nav-item dropdown">
                                        <a class="nav-link dropdown-toggle" href="03.Projects.html" id="dropdown02" >PORTFOLIO</a>
                                        <div class="dropdown-menu" aria-labelledby="dropdown02">
                                            <a class="dropdown-item" href="04.ProjectPresentation.html">PROJECT PRESENTATION</a>
                                        </div>
                                    </li>
                                    <li class="nav-item dropdown">
                                        <a class="nav-link dropdown-toggle" href="#" id="dropdown03" >BLOG</a>
                                        <div class="dropdown-menu" aria-labelledby="dropdown03">
                                            <a class="dropdown-item" href="05a.Blog.html">BLOG RIGHT SIDEBAR</a>
                                            <a class="dropdown-item" href="05b.BlogNoSidebar.html">BLOG NO SIDEBAR</a>
                                            <a class="dropdown-item" href="05c.BlogLeftSidebar.html">BLOG LEFT SIDEBAR</a>
                                            <a class="dropdown-item" href="06.BlogInner.html">BLOG POST</a>
                                        </div>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" href="07.Contact.html">CONTACT</a>
                                    </li>
                                </ul>
                                <form class="form-inline my-2 my-lg-0">
                                    <div class="cijo-search-wrapper">
                                        <input class="cijo-search-text" type="text" value="Search for...">
                                        <button type="button" class="cijo-search-button"></button>
                                    </div>
                                </form>
                            </div>
                        </nav>
                    </div>
                </div>
                <!-- /row -->
            </div>
        </header>
        <!-- /header -->
        <!-- main -->
        <main>
            <!-- section TEAM -->
            <section>
                <div class="container cijo-main-container">
                    <!-- row -->
                    <div class="row">
                        <div class="col-lg-6 col-md-10 col-12">
                            <h1>Who we are</h1>
                            <p class="cijo-desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas a justo mollis, feugiat lorem quis, euismod enim. Proin sed tortor lacus. Aenean id sapien nec ipsum tempor interdum sit amet sit amet nunc. Mauris a neque nec risus fermentum tempus. Aenean commodo mi blandit metus facilisis, in elementum odio consectetur. Phasellus sit amet libero non nisi efficitur ultricies. Vestibulum est sapien, dapibus vulputate maximus vel, blandit eu neque. Phasellus blandit molestie ligula nec ultrices. Cras dictum vestibulum facilisis. </p>
                        </div>
                    </div>
                    <!-- /row -->
                    <!-- row -->
                    <div class="row cijo-team-row">
                        <!-- col 1 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >MARIO SMITH</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-1.png" alt="MARIO SMITH" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 1 -->
                        <!-- col 2 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >NATALIE WELCH</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-2.png" alt="NATALIE WELCH" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 2 -->
                        <!-- col 3 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >JOHN LINCON</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-3.png" alt="JOHN LINCON" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 3 -->
                        <!-- col 4 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >SARAH LAWRENCE</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-4.png" alt="SARAH LAWRENCE" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 4 -->
                        <!-- col 5 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >CLAIRE HOLLAND</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-5.png" alt="CLAIRE HOLLAND" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 5 -->
                        <!-- col 6 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >MARIA VENTURA</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-6.png" alt="MARIA VENTURA" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 6 -->
                        <!-- col 7 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >JESSICA TORTURO</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-7.png" alt="JESSICA TORTURO" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 7 -->
                        <!-- col 8 -->
                        <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            <div class="cijo-team-thumb">
                                <div class="cijo-team-box">
                                    <div class="cijo-team-info" >
                                        <div class="cijo-team-social" >
                                            <a href="#" class="cijo-linkedin"><img src="assets/images/linkedin.svg" alt="linkedin"></a>
                                            <a href="#" class="cijo-email-icon"><img src="assets/images/email.svg" alt="email"></a>
                                        </div>
                                        <div class="cijo-team-name" >JACK NICKOLSON</div>
                                    </div> 
                                    <img class="img-responsive" src="assets/images/member-8.png" alt="JACK NICKOLSON" >                                     
                                </div>
                            </div>
                        </div>
                        <!-- /col 8 -->
                    </div>
                    <!-- /row -->
                </div>
            </section>
            <!-- /section TEAM -->
        </main>
        <!-- /main -->
        <!-- footer -->
        <footer class="bg-black text-white">
            <div class="container cijo-main-container">
                <div class="row cijo-footer-main-row">
                    <div class="col-12 mb-4 align-self-top">
                        <div class="row">
                            <h2 class="cijo-newsletter-h2">Sign up to our Newsletter</h2>
                        </div>
                        <div class="row cijo-newsletter-wrapper">
                            <input class="cijo-newsletter-input" type="text" value="Your email" />
                            <button class="cijo-button-arrow-right"></button>
                        </div>
                        <div class="row cijo-social">
                            <a href="#" class="cijo-social-link fb" >facebook</a>
                            <a href="#" class="cijo-social-link inst" >instagram</a>
                            <a href="#" class="cijo-social-link tw" >twitter</a>
                        </div>
                        <div class="row cijo-footer-contact">
                            <a href="#" class="cijo-email" >info@cijo.com</a><span class="cijo-separator">|</span><a href="#" class="cijo-phone" >+00 1 718 893 3900</a>
                        </div>
                    </div>
                    <div class="col-12 text-center col-sm-6 text-sm-left align-self-end">
                        <p>© 2020 All Rights Reserved by CiJo</p>
                    </div>
                    <div class="col-12 text-center col-sm-6 text-sm-right align-self-end">
                        <p><a href="#" >Terms of use</a> | <a href="#" >Privacy Policy</a></p>
                    </div>
                </div>
            </div>
        </footer> 
        <!-- /footer -->
        <!-- scripts -->
        <script src="assets/js/libs/jquery/jquery.slim.min.js"></script>
        <script src="assets/js/libs/bootstrap/bootstrap.bundle.min.js" ></script>
        <script src="assets/js/main.js" ></script>
        <!-- /scripts -->
    </body>
</html>